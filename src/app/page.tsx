...
export const metadata = {
  title: "메인 - 실시간 드래프트 시작하기",
  description: "미니언즈 경매 시스템으로 공정한 팀 구성과 실시간 드래프트를 경험하세요.  bananas included! 🍌",
};
      ...
      <div className="relative z-10 flex flex-col items-center px-4 py-20 gap-20">
        <div className="bg-white p-10 lg:p-14 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] max-w-3xl w-full text-center space-y-10">
          ...
          <h1 className="text-4xl lg:text-6xl font-heading flex flex-col items-center gap-4">
            <div className="flex items-center gap-3">
              <Image src="/favicon.png" alt="Icon" width={48} height={48} className="pixelated" />
              <div className="w-[450px]">
                <span className="text-minion-yellow block drop-shadow-[4px_4px_0px_rgba(0,0,0,1)]">MINIONS</span>
                <span className="drop-shadow-[4px_4px_0px_rgba(251,224,66,1)]">AUCTION</span>
              </div>
              <Image src="/favicon.png" alt="Icon" width={48} height={48} className="pixelated" />
            </div>
          </h1>
        ...
        <div className="max-w-6xl w-full space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {HOW_TO_USE.map((item) => (
              <div key={item.step} className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none transition-all">
                <div className="flex justify-between items-start mb-4">
                  <span className="text-3xl">{item.icon}</span>
                  <span className="bg-minion-yellow border-2 border-black px-2 py-0.5 text-[10px] font-bold">STAGE {item.step}</span>
                </div>
                <h3 className="text-lg font-black mb-2">{item.title}</h3>
                <p className="text-xs text-gray-600 font-bold leading-relaxed">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-3xl w-full bg-black text-white border-4 border-minion-yellow p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)]">
          ...
          <ul className="text-sm space-y-3 font-bold text-gray-300">
            <li className="flex gap-2"><span>-</span> <span>팀장 링크와 주최자 링크는 별개의 고유 주소입니다.</span></li>
            <li className="flex gap-2"><span>-</span> <span>모든 입찰은 실시간으로 동기화되며 취소가 불가능합니다.</span></li>
            <li className="flex gap-2"><span>-</span> <span>방 페이지 상단의 [LINK] 버튼으로 언제든 주소를 재확인하세요.</span></li>
          </ul>
        </div>
      </div>

      <footer className="py-10 text-center text-xs font-bold text-gray-500">
        Copyright © {new Date().getFullYear()} MINIONS(소모임). All rights reserved.
      </footer>
    </div>
  );
}
