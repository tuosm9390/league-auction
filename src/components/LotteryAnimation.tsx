...
export function LotteryAnimation({
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center gap-6 animate-in fade-in zoom-in-95 duration-500 py-4">
      <div
        className={`text-xl font-black tracking-widest transition-all duration-500 ${isSpinning
      ...
      >
        ...
        >
          {trackItems.map((p, idx) => (
            ...
            >
              <span
                className={`text-2xl font-black w-full text-center truncate transition-all duration-300 ${!isSpinning && idx === 40
                  ? 'text-minion-blue scale-110'
                  : 'text-gray-400'
                  }`}
                title={p.name}
              ...
              <span className={`text-xs font-bold transition-all duration-300 ${!isSpinning && idx === 40 ? 'text-minion-blue/60 scale-110' : 'text-gray-300'
                }`}
              >
