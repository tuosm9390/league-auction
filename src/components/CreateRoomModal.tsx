  ...
  return (
    <>
      <button
        ...
        className="bg-primary hover:bg-primary-hover text-secondary font-bold py-4 px-10 rounded-2xl text-2xl transition-all shadow-[0_6px_0_var(--color-primary-foreground)] hover:shadow-[0_4px_0_var(--color-primary-foreground)] hover:translate-y-1 active:shadow-none hover:scale-105 active:scale-95"
      >
        새로운 경매방 만들기
      </button>
      ...
      {isOpen && (
        ...
        >
          <div
            className="bg-popover rounded-3xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200 cursor-default"
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                <Image
                  src="/favicon.png"
                  alt="Minions Icon" className="dark:invert"
                  width={28}
                  height={28}
              ...
              {step < 3 && (
                <button
                  onClick={close}
                  className="text-muted-foreground hover:text-muted-foreground p-2 rounded-xl hover:bg-gray-100 transition-colors"
                >
                  <X size={20} />
                </button>
              )}
            ...
            <div className="px-6 pt-4 pb-2 flex items-center shrink-0">
              {STEPS.map((label, i) => (
                ...
                >
                  ...
                  <span
                    className={`ml-1.5 text-xs font-medium whitespace-nowrap ${i === step ? "text-secondary" : "text-muted-foreground"}`}
                  >
                    {label}
                  </span>
                  {i < STEPS.length - 1 && (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              ...
              {step === 0 && (
                <div className="space-y-5">
                  {/* 미완료 방 알림 배너 */}
                  {isCheckingRooms && (
                    <div className="bg-muted/50 border border-border rounded-2xl p-3 text-xs text-muted-foreground text-center">
                      이전 경매방 확인 중...
                    </div>
                  )}
                  {!isCheckingRooms && activeRooms.length > 0 && (
                    <div className="bg-muted/50 border border-border rounded-2xl p-4">
                      <p className="text-sm font-black text-orange-500 mb-3">
                        ⚠️ 진행 중인 경매방이 있습니다
                      </p>
                      <div className="space-y-2">
                        {activeRooms.map((room) => (
                          <div
                            key={room.id}
                            className="bg-popover border border-border rounded-xl p-3 flex items-center justify-between gap-3"
                          >
                            ...
                            <button
                              ...
                              className="flex items-center gap-1.5 bg-muted/500 hover:bg-orange-600 text-white px-3 py-2 rounded-xl text-xs font-bold transition-colors whitespace-nowrap shrink-0"
                            >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-muted border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-muted border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary"
                      />
                      <p className="text-xs text-muted-foreground mt-1">팀장 포함</p>
                    </div>
                    <div>
                      <label className="text-sm font-bold text-foreground block mb-1.5">
                        팀당 총 포인트
                  ...
                  <div>
                    ...
                    <div className="bg-muted/50 border border-border rounded-2xl p-4 text-xs text-muted-foreground leading-relaxed">
                  ...
                  <div className="bg-muted/50 rounded-2xl p-4 text-sm text-muted-foreground space-y-1">
                    <p className="font-bold text-secondary mb-1">요약</p>
                    <p>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-3 gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 bg-purple-500/10 hover:bg-purple-200 text-purple-500 px-3 py-1.5 rounded-xl text-sm font-bold transition-colors whitespace-nowrap shrink-0"
                    >
                      🎲 테스트 데이터 생성
                    </button>
                  </div>
                  {captains.map((captain, i) => (
                    <div
                      key={i}
                      className="border border-border rounded-2xl p-4 bg-muted/50"
                    >
                      <div className="flex items-center gap-2 mb-3">
                        <div className="w-7 h-7 bg-secondary rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0">
                          {i + 1}
                        </div>
                        <input
                          type="text"
                          ...
                          placeholder="팀 이름"
                          className="font-bold text-secondary bg-transparent border-b-2 border-border focus:border-secondary outline-none px-1 py-0.5 text-sm flex-1 min-w-0"
                        />
                        <span className="text-xs text-muted-foreground whitespace-nowrap shrink-0">
                          시작 포인트:{" "}
                          <span
                            className={`font-bold ${basic.totalPoints - captain.captainPoints > 0 ? "text-secondary" : "text-red-500"}`}
                          >
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        <div>
                          ...
                          <input
                            ...
                            placeholder="이름"
                            className="w-full bg-muted border border-border rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary bg-popover"
                          />
                        </div>
                        <div>
                          <label className="text-xs text-muted-foreground block mb-1">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={openTemplateModal}
                        className="flex items-center gap-1.5 bg-purple-500/10 hover:bg-purple-200 text-purple-500 px-3 py-1.5 rounded-xl text-sm font-bold transition-colors whitespace-nowrap"
                      >
                        🎲 테스트 데이터 생성
                      </button>
                      <input
                  ...
                  <div className="space-y-1.5">
                    ...
                    {players.map((player, i) => (
                      <div
                        key={i}
                        className="grid gap-2 items-center bg-muted/50 rounded-xl px-2 py-1.5"
                        style={{
                          gridTemplateColumns: "1.5rem 1fr 5rem 5rem 5rem 1fr",
                        }}
                      >
                        ...
                        <input
                          ...
                          placeholder="선수 이름"
                          className="border border-border rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary bg-popover w-full"
                        />
                        <select
                          value={player.tier}
                          onChange={(e) =>
                          ...
                          className="border border-border rounded-lg px-1 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-secondary bg-popover w-full"
                        >
                          {TIERS.map((t) => (
              ...
              {step === 3 && links && (
                <div className="space-y-4">
                  <div className="bg-green-500/10 border border-green-200 rounded-2xl p-4 text-center">
              ...
              {step < 3 ? (
                <>
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl text-sm font-bold text-muted-foreground hover:bg-gray-100 transition-colors"
                  >
                    {step === 0 ? "취소" : "← 이전"}
                  </button>
                  <button
                    ...
                    className="bg-secondary hover:bg-secondary-hover text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isLoading
                      ? "생성 중..."
                      : step === 2
                ...
                <>
                  ...
                  {links && (
                    <button
                      ...
                      className="bg-primary hover:bg-primary-hover text-secondary px-6 py-2.5 rounded-xl text-sm font-black transition-colors flex items-center gap-2 shadow-[0_4px_0_var(--color-primary-foreground)] hover:shadow-[0_2px_0_var(--color-primary-foreground)] hover:translate-y-0.5 active:shadow-none active:translate-y-1"
                    >
      ...
      {isTemplateModalOpen && templateData && (
        ...
        >
          <div
            className="bg-popover rounded-3xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* 헤더 */}
            <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
              ...
              <div>
                ...
                <div className="rounded-xl overflow-hidden border border-border">
                  <div
                    className="grid text-xs font-bold text-muted-foreground bg-muted/50 px-3 py-2"
                  ...
                  {templateData.captains.map((c, i) => (
                    <div
                      key={i}
                      className="grid text-xs text-foreground px-3 py-2 border-t border-gray-50 hover:bg-muted/50"
                      style={{ gridTemplateColumns: "2rem 1fr 1fr 4rem 1fr" }}
                    >
                      <span className="text-muted-foreground">{i + 1}</span>
                      <span className="font-bold text-secondary truncate pr-2">
                        {c.teamName}
                      </span>
            ...
            <div className="px-6 py-4 border-t border-border flex justify-between items-center shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="bg-secondary hover:bg-secondary-hover text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-colors"
                >
                  템플릿 적용 ✓
                </button>
              </div>
...
function LinkCard({
  ...
  return (
    <div className="border border-border rounded-xl p-3 flex items-center gap-3 bg-muted/50">
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-colors whitespace-nowrap shrink-0 ${
          copied === linkKey
            ? "bg-green-500/100/10 text-green-500"
            : "bg-popover hover:bg-gray-100 text-muted-foreground border border-border"
        }`}
      >
        {copied === linkKey ? (
          <>
