      ...
      >
        ...
        <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-muted/50 rounded-t-xl">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-destructive/20 rounded-lg flex items-center justify-center border border-destructive/30 text-destructive">
              <Trash2 size={16} />
            </div>
            <div>
          ...
          {!isCompleted && (
            <label className="flex items-start gap-2.5 cursor-pointer select-none group mt-1">
              <input
                ...
                className="mt-0.5 w-3.5 h-3.5 accent-destructive cursor-pointer rounded-sm"
              />
          ...
          {isCompleted ? (
            <>
              <button
                ...
                disabled={isDeleting}
                className="w-full py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-md font-bold transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm shadow-sm"
              >
                {isDeleting ? (
              ...
              <button
                ...
                disabled={isDeleting}
                className="w-full py-2.5 text-xs font-bold text-destructive hover:text-destructive hover:bg-destructive/10 border border-destructive/20 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
              >
                저장 없이 방 종료 (데이터 삭제)
              </button>
            </>
            ...
            <button
              ...
              className="w-full py-2.5 bg-destructive hover:bg-destructive/90 text-destructive-foreground rounded-md font-bold transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm shadow-sm"
            >
              {isDeleting ? (
                <span className="animate-spin text-base">⏳</span>
              ) : (
