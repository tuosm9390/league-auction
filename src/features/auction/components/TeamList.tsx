...
export function UnsoldPanel() {
  ...
  if (unsoldPlayers.length === 0)
    return (
      <div className="flex-1 flex justify-center items-center py-6 text-sm text-gray-400 font-bold italic">
        유찰 선수가 없습니다.
      </div>
    );
    ...
    <div className="flex flex-col gap-1 pb-1 w-full">
      ...
      <div className="grid grid-cols-1 gap-1">
        {unsoldPlayers.map((p: Player) => (
          ...
          >
            <span className="font-black text-gray-800 text-[12px] truncate mr-2">
              {p.name}
            </span>
            <span
              className={`text-[9px] font-black bg-white px-1.5 py-0.5 rounded border border-red-100 ${TIER_COLOR[p.tier] || "text-gray-400"}`}
            >
              {p.tier}
            </span>
          </div>
...
export function TeamList() {
  ...
  if (teams.length === 0)
    return (
      <div className="text-gray-400 text-sm text-center py-10 font-bold">
        팀 정보가 없습니다.
      </div>
    );
          ...
          >
            ...
            <div className="space-y-1.5">
              <p className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1.5 flex justify-between items-center px-0.5">
                ...
                <span
                  className={
                    isTeamComplete ? "text-green-500" : "text-gray-400"
                  }
                >
